...
import { Terminal, ArrowLeft, Copy, Check, AlertTriangle } from "lucide-react";
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="flex items-start gap-4">
        <div className="p-2 rounded-lg bg-primary/10"><Terminal className="h-6 w-6 text-primary" /></div>
        <div>
        ...
        <CardContent className="space-y-2">
          {cmd.commands.map((c, i) => (
            <div key={i} className="flex items-center gap-2 bg-muted rounded-md px-3 py-2 group">
              ...
              <button onClick={() => copyToClipboard(c, i)} className="opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <Card>
        ...
        <CardContent><p className="text-sm text-muted-foreground">{cmd.explanation}</p></CardContent>
      </Card>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {cmd.risks.length > 0 && (
          <Card className="border-amber-500/20 bg-amber-500/5">
            <CardHeader><CardTitle className="text-base text-amber-400">Risks</CardTitle></CardHeader>
            <CardContent>
